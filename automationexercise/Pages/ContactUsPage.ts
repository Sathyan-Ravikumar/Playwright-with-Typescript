...
export class ContactUsPage {
  ...
  async fillContactForm(
    ...
    await this.locators.subjectInput.waitFor({ state: "visible" });

    await this.locators.subjectInput.fill(subject);
    await this.locators.messageTextarea.waitFor({ state: "visible" });

    await this.locators.messageTextarea.fill(message);
    if (filePath) {
      await this.locators.fileUploadInput.waitFor({ state: "visible" });
